...
def calculate_signal():
    ...
    if last["volume"] > avg_vol * VOLUME_MULTIPLIER:
        long_score += 25
        short_score += 25
        reasons.append("C 爆量成立")

    # E 結構突破
    if last["close"] > prev_high:
        long_score += 25
        reasons.append("E 突破前高，偏多")
    if last["close"] < prev_low:
        short_score += 25
        reasons.append("E 跌破前低，偏空")

    # F 均線排列
    if ema20 > ema50 and last["close"] > ema20:
        long_score += 25
        reasons.append("F 多頭均線排列")
    if ema20 < ema50 and last["close"] < ema20:
        short_score += 25
        reasons.append("F 空頭均線排列")

    # H 壓力支撐
    mid = (prev_high + prev_low) / 2
    if last["close"] > mid:
        long_score += 25
        reasons.append("H 站上區間中線，支撐偏多")
    if last["close"] < mid:
        short_score += 25
        reasons.append("H 跌破區間中線，壓力偏空")
...
def calc_quantity():
    ...
    return qty, available, price, order_usdt

def place_order(direction):
    qty, available, price, order_usdt = calc_quantity()
...
@app.route("/")
def home():
    return "BTC BingX Bot 自動策略版啟動成功！"
...
@app.route("/positions")
def positions():
    return jsonify(bingx_get("/openApi/swap/v2/user/positions", {"symbol": SYMBOL}))
